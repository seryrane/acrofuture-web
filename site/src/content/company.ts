...
export const COMPANY = {
  ...
  /**
   * 업무 포탈 — 임직원용. **아크로퓨처 PoC 포털**(사내 업무 플랫폼) 서비스 서버.
   * 사용자 지정 2026-08-06 · 확인 시 200(문서 제목 "Acrofuture Portal").
   *
   * ⚠ **http 다(TLS 없음).** 브라우저가 "안전하지 않음"으로 표시하고, 이 주소에서 로그인하면
   *   사번·비밀번호가 암호화 없이 지나간다. 회사 소개 사이트에서 링크로 여는 것 자체는 막히지
   *   않지만, 임직원이 실제로 로그인하는 곳이므로 **https 로 올리는 것을 권한다.**
   * ⚠ IP+포트라 서버를 옮기면 이 값도 같이 바꿔야 한다. 도메인을 붙이면 그 일이 없어진다.
   */
  portalUrl: 'http://211.236.39.250:3400/',
